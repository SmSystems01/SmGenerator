<p align="center">
  <h1 align="center">⚡ SM-GENERATOR</h1>
</p>

<p align="center">
  <b>Fast and Valid Credit Card Generator using Luhn Algorithm.</b><br>
  <i>No external dependencies - Works everywhere!</i>
</p>

---

## 🚀 Features
- **Luhn Algorithm:** Generates 100% mathematically valid card numbers.
- **Custom BIN Support:** Generate cards based on your specific BIN.
- **Fast Generation:** Can generate 10,000+ cards in seconds.
- **Cross-Platform:** Works on Termux, Linux, Windows, and macOS.

## 📥 Installation

```bash
git clone [https://github.com/SmSystems01/SmGenerator.git](https://github.com/SmSystems01/SmGenerator.git)
cd SmGenerator
python smgenerator.py
